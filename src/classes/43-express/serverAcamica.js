...
server.get("/acamica/:comision/alumnos", (req, res) => {
  ...
  let alumnosPorComision = datos.alumnos.filter(
    (a) => a.comision === req.params.comision
  );

  if (req.query && req.query.nombre) {
    alumnosPorComision = alumnosPorComision.filter(
      (a) => a.nombre === req.query.nombre
    );
  }
...
server.get("/acamica/:comision/alumnos/:id", (req, res) => {
  const alumno = datos.alumnos.find(
    (a) =>
      a.id === parseInt(req.params.id) && a.comision === req.params.comision
  );
...
server.delete("/acamica/:comision/alumnos/:id", (req, res) => {
  const index = datos.alumnos.findIndex(
    (a) =>
      a.id === parseInt(req.params.id) && a.comision === req.params.comision
  );
